...
Modified by JCC for use in BMP project:
    - Added code to improve parsing of bytes.
      This included mostly extract_padding in some classes.
    - Changed BGPPACommunity to support multiple four octet values.
"""
...
class BGPHeader(Packet):
    ...
    def extract_padding(self, p):
        payload_length = self.len -  (16 + 2 + 1)
        return p[:payload_length], p[payload_length:]
...
class BGPOptParam(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            # According to https://tools.ietf.org/html/rfc5492, this parameter
            # can have "one or more triples". I am changing it from a PacketField to a PacketListField.
            PacketListField(
                "param_value",
                [],
                BGPCapability
...
class BGPPACommunity(Packet):
    ...
    References: RFC 1997
    According to 1997, "The attribute consists of a
    set of four octet values", not only one.
    TODO: Add the ASN separation
    """

    name = "COMMUNITIES"
    fields_desc = [
        FieldListField("communities", [], IntEnumField("community", 0, well_known_communities))]
